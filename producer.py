...
# Konfiguracja połączenia z Kafką
...
# Krok 1: Generowanie puli 10 000 unikalnych kart
print("Generowanie bazy 10 000 kart płatniczych...")
CARDS = []
for i in range(1, 10001):
    card_id = f"CARD_{i:05d}"
    user_id = f"USER_{random.randint(1, 6000)}"  # Niektórzy użytkownicy mają >1 kartę
...
def generate_normal_data(card):
    ...
    return {
        ...
        "gps": {
            "lat": round(random.uniform(50.0, 54.0), 4),  # Współrzędne PL
...
print("Symulator uruchomiony. Transakcje trafiają do Kafki. Naciśnij Ctrl+C, aby zatrzymać.")

# Krok 2: Główna pętla symulacji ruchu
try:
    while True:
        card = random.choice(CARDS)
        rand_action = random.random()

        # 97% szans - transakcja standardowa
        if rand_action > 0.03:
            tx = generate_normal_data(card)
            PRODUCER.send(TOPIC, key=tx["card_id"].encode('utf-8'), value=tx)
        
        # 3% szans - generowanie losowej anomalii (zgodnie z wymaganiami projektu)
        else:
            anomaly_type = random.choice(["high_amount", "impossible_travel", "carding"])
            
            if anomaly_type == "high_amount":
                # Anomalia 1: Nagła, ogromna kwota blisko limitu karty
                tx = generate_normal_data(card)
                ...
                # Anomalia 2: Szybka zmiana lokalizacji (Najpierw PL, zaraz potem Nowy Jork)
                tx1 = generate_normal_data(card)
                PRODUCER.send(TOPIC, tx1)
                
                tx2 = generate_normal_data(card)
                tx2["gps"] = {"lat": 40.7128, "lon": -74.0060}  # Nowy Jork
                ...
                # Anomalia 3: Częstotliwość (Seria drobnych transakcji w ułamku sekundy)
